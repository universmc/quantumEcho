...
async function main() {
  ...
  const chatCompletion = await groq.chat.completions.create({
    "messages": [
      ...
      {role: "assistant",name:"[📔.codex]", content:"phase[01-2]:[RUN]init{config[]:[groq `https://Ia.univers-mc.mc/DisKetCrypto/kernel/usr/ia/qumtumEcho/`)]"},
      {role: "assistant",name:"[📔.codex]", content:"phase[01-3]:[RUN]init{role:assistant,name:`[💻.quantumEcho]`content:prompt:[groq `https://chat.openai.com/g/g-715QHIlVy-disketcrypto`)]"},
      {role: "system",name:"[🌌.systemDream]", content:"Elaboration de HOWTO"},
      
      {role: "user",name:"[🌴.qroq]", content:"Howto make bootstrap components"},
      {role: "assistant",name:"[📔.codex]", content:"phase[01-4]:[RUN]init{devOps}"},
  ...
  //  {role: "system",name:"[🌌.systemDream]", content:"groq"},
    ...
    const outputFilePath = "output/quantumEcho_" + new Date().toISOString().replace(/[-:TZ]/g, "") + ".md";
